...
function cacheElements() {
  ...
  elements.navModeAnalytics = document.getElementById('nav-mode-analytics');
  elements.tabBtnChat = document.getElementById('tab-btn-chat');
  elements.tabBtnImages = document.getElementById('tab-btn-images');
  elements.tabBtnAnalytics = document.getElementById('tab-btn-analytics');
  ...
  elements.growthAnalyticsView = document.getElementById('growth-analytics-view');
...
function saveChatsToStorage() {
  ...
  try {
    window.dispatchEvent(new CustomEvent('godgpt:chats-updated'));
  } catch (e) {
...
function saveGalleryToStorage() {
  ...
  try {
    window.dispatchEvent(new CustomEvent('godgpt:gallery-updated'));
  } catch (e) {
    // Ignore in non-browser context
  }
}
...
function setupEventListeners() {
  ...
  const handleViewSwitch = (mode) => {
    ...
    elements.growthAnalyticsView?.classList.add('hidden-view');

    // Deactivate all nav tabs
    elements.navModeChat?.classList.remove('active');
    elements.navModeImages?.classList.remove('active');
    elements.navModeAnalytics?.classList.remove('active');
    ...
    if (mode === 'chat') {
      ...
      elements.growthAnalyticsView?.classList.remove('hidden-view');
      elements.navModeAnalytics?.classList.add('active');
      elements.tabBtnAnalytics?.classList.add('active');
      // Trigger update event to ensure analytics data is fresh
      try {
        window.dispatchEvent(new CustomEvent('godgpt:chats-updated'));
      } catch (e) {}
    }
  ...
  elements.navModeChat?.addEventListener('click', () => {
    handleViewSwitch('chat');
    closeSidebar();
  });
  elements.navModeImages?.addEventListener('click', () => {
    handleViewSwitch('images');
    closeSidebar();
  });
  elements.navModeAnalytics?.addEventListener('click', () => {
  ...
  elements.tabBtnAnalytics?.addEventListener('click', () => handleViewSwitch('analytics'));
...
function createMessageElement(msg) {
  ...
  const avatar = isUser
    ...
    : `<div class="avatar ai-avatar"><img src="logo.png" alt="GOD GPT" onerror="this.onerror=null; this.src='logo.svg';" /></div>`;
...
async function attemptDirectKeyRequest(apiKey, promptText, historyTurns) {
  ...
  const isPro = Boolean(state.activeModel && state.activeModel.toLowerCase().includes('pro'));
  const sysMsg = isPro
    ? 'You are GOD GPT Pro v2.0, an elite AI assistant created and owned by Preatom YT. When the user sends a greeting (like "Hi", "Hello", "Hey", "Assalamu Alaikum"), reply with a warm welcome message stating "Created & Owned by Preatom YT" and explain how GOD GPT Pro v2.0 helps with general AI tasks, coding, writing, research, and analysis.'
    : 'You are GOD GPT v2.0, the ultimate Social Media AI Master created and owned by Preatom YT. When the user sends a greeting (like "Hi", "Hello", "Hey", "Assalamu Alaikum"), reply with a warm welcome message stating "Created & Owned by Preatom YT" and explain in detail how GOD GPT v2.0 helps master Social Media (viral hooks, organic growth, algorithm news, monetization, thumbnails).';
